...
class RiaParser:
    def __init__(self, client_pool: list[AsyncClient]):
        self._repository = RiaRepository()
        self._ria_link_repository = RiaLinkRepository()
    ...
    async def run(self) -> None:
        ...
        async for batch in self._ria_link_repository.stream(status=ParseStatus.NEW, link_type=LinkType.CARD):
            # ---------- PARSE CARDS ----------
            ...
            # ---------- SAVE BATCH CARDS TO DB ----------
            await self._repository.create_bulk_without_conflict(cards, index_elements=["url"])

            proceed_card_links = [card["url"] for card in cards]
            await self._ria_link_repository.update_status_by_urls(proceed_card_links, ParseStatus.PROCEED)


    async def retry_failed_links(self) -> None:
        print("RETRY FAILED LINKS...")
        async for batch in self._ria_link_repository.stream(status=ParseStatus.ERROR, link_type=LinkType.COMMON):
            await asyncio.gather(*[self._parse_retry_card_links(link_model.url, len(batch)) for link_model in batch])

        async for batch in self._ria_link_repository.stream(status=ParseStatus.ERROR, link_type=LinkType.CARD):
    ...
    async def _parse_retry_card_links(self, url: str, total_pages: int):
        ...
        req_el = await self._ria_link_repository.get_all(url=url, status=ParseStatus.ERROR, link_type=LinkType.COMMON)
        req_el = req_el[0]
        async with self._semaphore:
            try:
                response = await choice(self._client_pool).get(url=url)
            except httpx.HTTPError:
                link_schema = LinkObserverSchema(
                    url=url,
                    status=ParseStatus.ERROR,
                    link_type=LinkType.COMMON,
                    error_message=traceback.format_exc()
                )
                link_schema.status = ParseStatus.ERROR
                if req_el.count_retries == 3:
                    link_schema = ParseStatus.DEAD
                else:
                    link_schema.count_retries += 1
                await self._ria_link_repository.update(req_el.id, link_schema.model_dump(exclude_none=True))
                return

        root = fromstring(response.content)
        links = self._card_parser.parse_card_links(root)
        print(f"INSERTING {len(links)} COMMON LINKS...")
        model_obj = { "url": None, "status": ParseStatus.NEW, "link_type": LinkType.CARD }
        els = []
        for link in links:
            el = model_obj.copy()
            el["url"] = link
        ...
        await self._ria_link_repository.create_bulk_without_conflict(els, index_elements=["url"])
        await self._ria_link_repository.update(req_el.id, {"status": ParseStatus.PROCEED, "error_message": None})
    ...
    async def _parse_card_links(self, page_number: int, total_pages: int) -> None:
        ...
        async with self._semaphore:
            try:
                ...
                await self._ria_link_repository.create_without_conflict(
                    link_schema.model_dump(exclude_none=True), index_elements=["url"]
                )
                return
    ...
    @httpx_retry_on_failure()
    async def _parse_card(self, link_model: RiaLinkObserverModel, total_cards: int) -> dict:
        ...
        async with self._semaphore:
            try:
                ...
                if new_values["count_retries"] > 3:
                    new_values["status"] = ParseStatus.DEAD

                await self._ria_link_repository.update(link_model.id, new_values)
                return {}
